...
#define dcdc_phase1 (PWM_TBPRD_dcdc/3.0)*3.0
#define dcdc_phase2 (PWM_TBPRD_dcdc/3.0)*2.0
#define dcdc_phase3 (PWM_TBPRD_dcdc/3.0)*1.0
...
uint16_t a, b, c, d, e, g, h;// con la f no funciona xd
uint16_t contador_CMPB = 0;

//
void InitEPwm1Example(void);
Uint32 EPwm1TimerIntCount;
Uint16 EPwm1_DB_Direction;
//---------Funciones_y_definiciones_UART---------//
...
//---------------CTRL 1 y CTRL 2-------------------------
//   se debe activar para probar el gabinete blower retrofit de taller
...
//    EPwm1Regs.ETSEL.bit.SOCBEN  = 1; // Habilita pulso SOCB  (EPWMxSOCB)
    ...
    EPwm3Regs.TBCTL.bit.CTRMODE  = TB_COUNT_UP; // Contador entra en up count mode
    EPwm4Regs.TBCTL.bit.CTRMODE  = TB_COUNT_UP; // Contador entra en up count mode
    EPwm7Regs.TBCTL.bit.CTRMODE  = TB_COUNT_UP; // Contador entra en up count mode
...
//                    if (contador_CMPB < PWM_TBPRD_dcdc*0+100)
//                    {
//                        contador_CMPB = contador_CMPB + 1;
//                    }
//                    else if (contador_CMPB = PWM_TBPRD_dcdc*0+100)
//                    {
//                        contador_CMPB = PWM_TBPRD_dcdc*.9;
//                    }
...
void EPWM_initEpwm(void)
{
    EPwm1Regs.TBPRD           = PWM_TBPRD_dcdc;  // Frecuencia de 2khz
    EPwm1Regs.TBCTL.bit.PHSEN = TB_DISABLE;        // Deshabilitar sincronización de fase
    ...
    // Setup compare
    EPwm1Regs.CMPA.bit.CMPA = PWM_TBPRD_dcdc/2;
...
//    EPwm1Regs.ETPS.bit.SOCAPRD    = 1;    // Genera el pulso en el 1st evento    //
    // Configuración eWPM3
    //
    EPwm3Regs.TBPRD           = PWM_TBPRD_dcdc;  // Frecuencia de 2khz
    EPwm3Regs.TBCTL.bit.PHSEN = TB_ENABLE;        // Enable phase loading
    EPwm3Regs.TBPHS.bit.TBPHS = dcdc_phase1;           // Phase is 0
...
//    EPwm3Regs.ETSEL.bit.SOCASEL   = 0x6;    // Genera Evento cuando TBCTR = 0x00 (inicio conversión)
...
//    EPwm4Regs.TBCTL.bit.SYNCOSEL = TB_SYNC_DISABLE; // sync flow-through, slave mode
    ...
    // Setup compare
    EPwm4Regs.CMPA.bit.CMPA = PWM_TBPRD_dcdc/2;
    ...
    // Configuración eWPM7
    //
    EPwm7Regs.TBPRD           = PWM_TBPRD_dcdc;  // Frecuencia de 2khz
    EPwm7Regs.TBCTL.bit.PHSEN = TB_ENABLE;        // Enable phase loading
//    EPwm7Regs.TBCTL.bit.PHSEN = TB_DISABLE;        // Enable phase loading
    EPwm7Regs.TBPHS.bit.TBPHS = dcdc_phase3;           // Phase is 0
...
//    EPwm7Regs.TBCTL.bit.SYNCOSEL = TB_SYNC_DISABLE; // sync flow-through, slave mode
    ...
    // Setup compare
    //
    EPwm7Regs.CMPA.bit.CMPA = PWM_TBPRD_dcdc/2;
...
//                EPwm3Regs.CMPA.bit.CMPA = (1-0.2)*PWM_TBPRD_dcdc;
            ...
            case 2:
            {
                EPwm7Regs.CMPA.bit.CMPA = x_i_g[i_count_l]*PWM_TBPRD_dcdc;
            }
            break;
        }
...
//    uint16_t a, b, c, d, e, g, h;// con la f no funciona xd
//
//        a = AdcaResultRegs.ADCRESULT0; // vhigh
////        b = AdcaResultRegs.ADCRESULT1; // vlow
////        c = AdcaResultRegs.ADCRESULT2; // I_ADC1
////        b = ((uint16_t)((AdcaResultRegs.ADCRESULT2)*m_i_2   + n_i_2)*100/18);
////        c = ((uint16_t)((AdcaResultRegs.ADCRESULT2)*m_i_2   + n_i_2)/15);
////        c = AdcaResultRegs.ADCRESULT2; // I_ADC1
////        g = AdcaResultRegs.ADCRESULT0;
////        h = AdcaResultRegs.ADCRESULT0;
//
////        c = 2000;
//        b = (uint16_t) resultado [1];
//        c = (uint16_t) resultado [2];
//        d = 2000;
//        e = 2000;
//        g = 2000;
//        h = 2000;
